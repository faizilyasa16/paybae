...
export default function TransactionItem({ icon: Icon, title, desc, date, amount, type, color, bg, border, amountClass = null }) {
    ...
    if (!finalAmountClass) {
        if (amount > 0) {
            ...
            finalAmountClass = 'text-slate-800 dark:text-white'; // default hitam jika kurang spesifik
        }
    }

    return (
        <div className="flex items-center justify-between p-3.5 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border border-slate-100/50 dark:border-slate-800/50 group">
            <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-[14px] ${bg} ${color} flex items-center justify-center text-xl shadow-sm dark:shadow-none border ${border} group-hover:scale-105 transition-transform flex-shrink-0`}>
                    {Icon && <Icon />}
                </div>
                <div>
                    ...
                    {date ? (
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">{desc}</p>
                    )}
                </div>
            </div>
